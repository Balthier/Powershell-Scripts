...
$MSOffice = Get-WmiObject -class Win32_product | Where-Object { $_.Description -Like 'Microsoft Office*' -And $_.Description -NotLike '*MUI*' -And $_.Description -NotLike '*Components*' -And $_.Description -NotLike '*Runtime*' -And $_.Description -NotLike '*Proof*' }
...
$dupeCheck = if ($csvCheck) {
 Get-Content $csv | Select-String $PCName 
}
...
function getOfficeKey() {
	...
	if ($null -eq $key) {
		...
		if ($null -eq $key) {
			Write-Host "MS Office is not installed."; break 
  }
	}
...
function getOfficeID($Arch) {
	...
	Get-ChildItem $Path -recurse -ea SilentlyContinue | Where-Object { (Get-ItemProperty -Path $_.PsPath -ea SilentlyContinue) -match "digitalproductid" -eq $True } |  ForEach-Object { 
		$reg = $_.PsPath 
